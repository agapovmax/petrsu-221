...
Полагаю, для этого надо изменить код возврата на -1 в функции **is_lychrel_candidate(long number)**

```c
/* Если сумма числа и его обращения переполняет разрядную сетку, */
49              if (n > LONG_MAX - r) {
50                  /* то считаем число искомым кандидатом и завершаем проверку */
51                  return -1;
52              }
```

Теперь вывод будет иной
...
Надо поменять код выхода return на вывод printf
